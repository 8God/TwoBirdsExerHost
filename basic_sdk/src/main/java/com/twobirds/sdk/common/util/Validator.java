...
//    }
    ...
    public static boolean isMobilePhoneNumber(final String str) {
        ...
        if (isEmpty(str) || !Pattern.matches(telRegex, str)) {
            isMobilePhoneNumber = false;
        }
    ...
    // 密码长度 : 最小长度--最小6位~最大22位;允许字符集：26个英文字母，数字0~9，键盘上常用的字符#@%等（英文半角码）
    public static boolean isPassword(final String str) {
        boolean isPassword = false;
        if (!isEmpty(str) && Pattern.matches("^[a-zA-Z0-9_#@%!?~%$&]{6,22}$", str)) {
            isPassword = true;
        }
        return isPassword;
    }
